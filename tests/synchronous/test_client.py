...
class TestClient(Client):
    ...
    def __init__(self, requests_mock):
        ...
        requests_mock.get("https://api.passport.yandex.ru/all_accounts",
                          json={"status": "OK"})
...
def test_get_lessons(requests_mock):
    ...
    for data in json.loads(fake_json):
        requests_mock.get(
            get_lessons_list_link(course_id=course.id,
                                  group_id=course.group.id),
            json=data,
        )
        try:
            assert client.get_lessons(course_id=course.id,
                                      group_id=course.group.id)
...
def test_get_lesson_information(requests_mock):
    ...
    for data in json.loads(fake_json):
        requests_mock.get(
            get_lesson_information_link(lesson_id=lesson.id,
                                        course_id=course.id,
                                        group_id=course.group.id),
            json=data,
        )
        assert client.get_lesson(lesson_id=lesson.id,
                                 course_id=course.id,
                                 group_id=course.group.id)
...
def test_get_tasks(requests_mock):
    ...
    for data in json.loads(fake_json):
        requests_mock.get(
            get_tasks_list_link(lesson_id=lesson_id,
                                course_id=course_id,
                                group_id=group_id),
            json=data,
        )
        tasks = client.get_tasks(lesson_id=lesson_id,
                                 course_id=course_id,
                                 group_id=group_id)
        if not tasks == []:
            assert tasks
